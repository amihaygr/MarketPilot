...
async function load() { toast.hidden=true; try { const symbolQuery=userState.symbols.length?`?symbols=${encodeURIComponent(userState.symbols.join(","))}`:""; const [response,evaluationResponse,alertsResponse]=await Promise.all([fetch(`${API}/opportunities${symbolQuery}`,{headers:{Accept:"application/json"}}),fetch(`${API}/decision-evaluation/status`),fetch(`${API}/decision-alerts?limit=20`)]); if(!response.ok||!evaluationResponse.ok||!alertsResponse.ok) throw new Error("one or more decision APIs failed"); const payload=await response.json(); items=payload.items; renderEvaluation(await evaluationResponse.json()); renderAlerts((await alertsResponse.json()).items); document.querySelector("#tracked").textContent=items.length; document.querySelector("#buy-count").textContent=items.filter(x=>x.action==="BUY ZONE").length; document.querySelector("#certified-count").textContent=items.filter(x=>x.lifecycle_status==="CERTIFIED").length; document.querySelector("#fresh-count").textContent=items.filter(x=>new Date(x.valid_until_utc)>new Date()).length; renderWatchlist(); } catch(error) { items=[]; renderWatchlist(); showError(`Opportunity data unavailable: ${error.message}`); } }
