...
7. **Backtesting Lab:** <http://localhost:3000/backtesting.html>
...
- ב־Airflow מצא את ה־DAG בשם `historical_market_backfill` ואת הריצה
  `phase12_month_lineage_final_20260905`.
- ב־Backtesting Lab ודא שמופיעים **20 sessions**, **23,349 observations**
  ו־**555 trades**.
- סגור את `.env`, טרמינלים שמציגים הגדרות וכל מקום שעלול לחשוף credentials.

## מפת המסלול במשפט אחד

`Project Story → Dashboard → Kafka → MinIO → Airflow → Backtesting → Project Story`
...
>
> MarketPilot אוספת נתוני מניות מ־Alpaca ודיווחים רשמיים של חברות מה־SEC,
> שהוא רשות ניירות הערך האמריקאית. היא הופכת את המידע לגרפים, מדדי SMA ו־RSI,
> תנודתיות, איתותים מוסברים ו־Backtesting מול SPY כמדד השוואה.
>
> אבל המוצר לא רק מציג מספרים יפים. מאחורי כל תוצאה נשמר המקור הגולמי,
> מופעלות בדיקות איכות, ויש אפשרות לדעת מאיזה Event, תהליך וגרסת קוד היא נוצרה.
...
- לקבל Indicators ואיתותים עם הסבר, ולא רק צבע ירוק או אדום.
...
לא בנית רק Dashboard ולא רק מספר Containers. בנית מוצר מחקר לסוחר, ומאחוריו
מסלול Data Engineering מלא שמייצר אמון בנתון.
...
**בחר:** `phase12_month_lineage_final_20260905`
...
> זו הריצה הסופית של Phase 12. היא עיבדה 20 ימי מסחר אמיתיים לפי לוח XNYS.
...
## שלב 5 — Backtesting: מה הנתונים מאפשרים לעשות
...
>
> ב־AAPL התקבלה תשואה של 0.19 אחוז מול Benchmark של 2.60 אחוז. התוצאה אינה
...
## שלב 6 — אמינות: מה קורה כשיש בעיה
...
## שלב 7 — סיכום: מה היו ההחלטות שלי
