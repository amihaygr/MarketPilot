...
>
> הקו המקווקו הוא <bdi dir="ltr"><code>SMA 20</code></bdi> — ממוצע של עשרים סגירות אחרונות. הוא לא אומר לי
...
> כאן הנתון נכנס למערכת בפעם הראשונה. <bdi dir="ltr"><code>Kafka</code></bdi> הוא לא המסד שממנו
> הדשבורד קורא. הוא יומן מסודר של הודעות.
>
> לכל הודעה יש מחיצה, שנקראת <bdi dir="ltr"><code>partition</code></bdi>, ומספר מיקום, שנקרא
> <bdi dir="ltr"><code>offset</code></bdi>. אני אוהב לחשוב עליהם כמו על מדף ומספר סידורי. לכן אפשר
...
2. לחץ על התיקייה <bdi dir="ltr"><code>source=alpaca</code></bdi>. המשמעות: מקור האירוע הוא ספק נתוני השוק Alpaca.
...
> הגולמי שנשמר לפני העיבוד המאושר. הנתיב עצמו מספר סיפור: הנתון הגיע מ־Alpaca,
> הוא נר של דקה, הוא שייך לתאריך ולמניה מסוימים, והוא נכתב מתוך נושא ומיקום
> מוגדרים ב־Kafka.
>
> בתוך הקובץ אני רואה שני זמנים שונים: <bdi dir="ltr"><code>event_time_utc</code></bdi> הוא הזמן שבו האירוע
> קרה בשוק, ו־<bdi dir="ltr"><code>ingested_at_utc</code></bdi> הוא הזמן שבו המערכת קלטה אותו. ההבחנה הזו
> מאפשרת לי לבדוק גם טריות ולא רק מחיר. <bdi dir="ltr"><code>schema_version</code></bdi> אומר באיזו גרסה של
> חוזה הנתונים השתמשנו, כדי ששינוי עתידי במבנה לא ישבור את העיבוד בשקט.
...
> Airflow הוא מנהל העבודה של התהליכים שמתחילים ומסתיימים. כאן רואים בדיוק
> מה קרה ובאיזה סדר: מעבדים נתונים מה־Bronze אל ה־Silver, בודקים איכות,
> מפרסמים ל־Gold ורק אחר כך ממשיכים לחישובי הניתוח. בריצת ההשלמה האחרונה
> עובדו 21 ימי מסחר.
>
> חשוב להדגיש מה Airflow לא עושה: הוא לא מפעיל את Kafka, לא מפעיל את Spark
> Streaming ולא מנהל את חיי הדשבורד. אלה שירותים ארוכי־חיים ש־Docker Compose
> מנהל. ההפרדה הזאת מונעת מצב שבו תזמון של עבודת Batch עוצר בטעות את המסלול החי.

**למה זה קיים:** בלי תזמור, קשה לדעת אם כל השלבים רצו לפי הסדר ואם נתון עבר
בדיקות איכות לפני שפורסם. Airflow נותן היסטוריית ריצות, סטטוס ותלויות.
...
> רווח.
>
> חשוב גם מה שהמערכת לא עושה: היא לא מבצעת פקודת קנייה. היא נמצאת ב־<bdi dir="ltr"><code>Shadow Mode</code></bdi>,
> כלומר אוספת ומודדת את איכות התרחישים לפני שמציגים אותם כהמלצה מעשית. האדם
> נשאר מקבל ההחלטה.
...
> לב. התוצאה כוללת עלויות ו־<bdi dir="ltr"><code>Slippage</code></bdi>, ואנחנו משווים אותה ל־<bdi dir="ltr"><code>SPY</code></bdi>, כדי שלא
