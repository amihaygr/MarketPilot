...
def archive_company_facts_payload(
    s3: Any, *, bucket: str, cik: str, payload: bytes, partition_date: date
) -> tuple[str, str]:
    digest = hashlib.sha256(payload).hexdigest()
    key = (
        "source=sec/event=company_facts/"
        f"year={partition_date:%Y}/month={partition_date:%m}/day={partition_date:%d}/"
        f"cik={cik}/sha256={digest}.json"
    )
    try:
        ...
        s3.put_object(
            ...
            Body=payload,
            ContentType="application/json",
            Metadata={"sha256": digest, "source": "sec-companyfacts"},
        )
    return f"s3://{bucket}/{key}", digest
