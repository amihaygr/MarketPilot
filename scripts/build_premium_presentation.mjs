...
// 1 — Cover
{
  ...
  text(
    slide,
    rtl(`סביבת מחקר אחת לסוחר\nמחברת נתונים, סיכון והסבר\nלתמונה שאפשר לסמוך עליה`),
    630,
    392,
    580,
    76,
  ...
  notes(
    slide,
    "00:00–00:30",
    "פתח כאדם שמציג בעיה שהוא באמת רצה לפתור: כשאני מסתכל על מניה, המחיר לבדו לא מספיק לי. אני רוצה לדעת אם הנתון טרי, מה השתנה בחברה, מה הסיכון והאם הרעיון עמד במבחן היסטורי. לכן בניתי סביבת מחקר שמרכזת את התמונה ומאפשרת לעקוב עד מקור הנתון. המערכת אינה שולחת פקודות מסחר ואינה מבטיחה תשואה.",
...
// 2 — Problem
{
  ...
  text(slide, rtl("המחיר הוא רק תחילת השאלה"), 420, 82, 790, 72, {
    size: 44,
    bold: true,
    color: C.white,
    align: "right",
  ...
  text(
    slide,
    rtl("האתגר אינו מחסור במידע.\nהאתגר הוא לחבר מחיר, איכות, מצב החברה וסיכון\nלתמונה אחת שאפשר להסביר."),
    530,
    176,
    680,
    74,
  ...
  const questions = [
    ...
    ["03", "מה השתנה בחברה?", "דיווחים רשמיים של החברה"],
    ["04", "האם הרעיון עבד בעבר?", "השוואה למדד השוק ועלויות"],
  ];
  ...
  notes(
    slide,
    "00:30–01:15",
    "ספר את הבעיה מנקודת המבט שלך כמשתמש: כדי לבחון מניה צריך לעבור בין גרף, חדשות, דיווחי חברה וכלי ניתוח, ועדיין לא תמיד ברור מה טרי ומה אמין. ארבע השאלות בתחתית הפכו לדרישות המוצר: להבין מה קורה, לבדוק את איכות הנתון, להוסיף הקשר עסקי ולבחון את הרעיון על העבר.",
...
// 3 — Product value
{
  ...
  text(
    slide,
    rtl(`התוצאה: פחות זמן בחיפוש ובהצלבת מסכים, ויותר זמן בהבנת ההזדמנות, הסיכון והסיבה להחלטה.`),
    238,
    460,
    978,
    92,
  ...
  text(slide, rtl(`המערכת תומכת בהחלטה בלבד. ההחלטה והביצוע נשארים בידי המשתמש.`), 520, 576, 696, 34, {
    size: 20,
    color: C.coral,
    align: "right",
  });
  notes(
    slide,
    "01:15–02:00",
    "הסבר את הערך בשפה עסקית: במקום לעבור בין כמה מקורות ולנסות לחבר הכול לבד, המשתמש מקבל סביבת מחקר אחת. המערכת מרכזת נתוני שוק ומידע רשמי, בודקת איכות, מוסיפה הקשר ומציגה תרחיש עם טווח כניסה, נקודת ביטול ויחס סיכון־סיכוי. היא מקצרת את זמן המחקר ומעלה את רמת השקיפות, אבל ההחלטה והביצוע נשארים בידי המשתמש.",
...
// 6 — Technology choices
{
  const slide = presentation.slides.add();
  base(slide);
  title(slide, "בחירה מודעת: כלי מתאים לכל אחריות", "TECHNOLOGY CHOICES", 6, false);
  const rows = [
    ["Kafka", "יומן אירועים בין המקור לצרכנים", "מפריד בין רכיבים ושומר היסטוריה; לא חיבור ישיר", C.teal],
    ["Spark", "מנוע אחד לעיבוד רציף ולעבודות אצווה", "אותו מנוע לשני מסלולים; לא שתי לוגיקות נפרדות", C.blue],
    ["Airflow", "תזמון ותלות בין עבודות שמסתיימות", "סטטוס, לוגים וניסיון חוזר; לא תזמון ידני", C.amber],
    ["MinIO", "אחסון גולמי וקבצים אנליטיים", "מותאם לקבצים ולארכיון; לא תיקייה מקומית או מסד הגשה", C.teal],
    ["MariaDB", "שכבת נתונים מוכנה לאפליקציה", "שאילתות ואינדקסים מהירים; לא אחסון קבצים", C.blue],
    ["Docker Compose", "מחזור החיים של השירותים המקומיים", "פשוט לשחזור מקומי; לא הפעלה ידנית או תשתית כבדה", C.amber],
  ];
  text(slide, rtl("למה היא, ולא החלופה"), 760, 132, 450, 28, { size: 17, bold: true, color: C.mutedDark, align: "right" });
  ...
  rows.forEach(([tech, role, why, accent], index) => {
    ...
    text(slide, rtl(role), 330, y, 390, 50, { size: 18, color: C.white, align: "right", valign: "middle" });
    text(slide, rtl(why), 750, y, 460, 50, { size: 17, color: C.mutedDark, align: "right", valign: "middle" });
    if (index < rows.length - 1) rule(slide, 64, y + 58, 1152, C.lineDark, 1);
  });
  text(slide, rtl("בחרתי בכוונה בכלים שלמדנו ונחשפנו אליהם בקורס, ואז חיברתי ביניהם לפי גבולות אחריות ברורים."), 300, 630, 910, 34, {
    size: 17,
    color: C.teal,
    align: "right",
  });
  notes(
    slide,
    "04:15–05:05",
    "פתח בכנות: בחרתי לעבוד עם הכלים שלמדנו ונחשפנו אליהם בקורס, אבל לא חיברתי אותם רק כדי לסמן וי. לכל כלי נתתי אחריות שמתאימה לחוזקות שלו. Kafka עדיף כאן על חיבור ישיר כי הוא מפריד בין המקור לצרכנים ומאפשר Replay. Spark חוסך שני מנועי עיבוד שונים למסלול החי ולאצווה. Airflow נותן תלויות, ניסיונות חוזרים ולוגים במקום אוסף משימות Cron. MinIO שומר מידע גולמי ו-Parquet בצורה שדומה ל-S3, במקום לערבב ארכיון עם מסד ההגשה. MariaDB מתאים לשאילתות SQL מהירות של האפליקציה, ו-Docker Compose נותן סביבה מקומית שחוזרת על עצמה בלי המורכבות של Kubernetes. אל תקריא את כל הטבלה; בחר שתיים או שלוש דוגמאות והסבר את שיקול הדעת.",
